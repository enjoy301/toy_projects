...
def getFingerPosition(max_contour, img_result, debug):
    ...
    max_contour = cv.approxPolyDP(max_contour, 0.02 * cv.arcLength(max_contour, True), True) # 외곽선 아주대충
    hull = cv.convexHull(max_contour) # 외곽선의 모든점 리턴
...
cascade = cv.CascadeClassifier("files/haarcascade_frontalface_alt.xml")
